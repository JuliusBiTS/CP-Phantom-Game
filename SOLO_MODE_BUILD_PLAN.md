...
> **Phase 1 build status (2026-08-31):** repo `JuliusBiTS/CP-Phantom-Game`. **The full Phase 1 mechanical layer is complete** — 50 passing tests, `tsc`/`build`/`lint` clean, PC-PW math verified end-to-end in the browser. Done:
> - Real dice engine (`src/lib/dice/rollPW.ts`) — CSPRNG, v12 §2.1 capped-remainder + crit semantics ported from CP Phantom's `rollInitiative`; tests include the v12 worked example and a distribution check.
> - v12 §7.7 PW-halving-stack ordering + range bands (`src/lib/rules/pw.ts`).
> - Ported `getWoundState` house rule (`src/lib/rules/woundState.ts`).
> - **Live PC-PW math** (`src/lib/rules/live.ts`) — ported `getEffectiveStats` / `computeLiveWeaponStats` / `computeLiveReactionStats` + weapon/cyberware/armor/Smart-tier catalogs → a `pcPwReference` block the turn prompt injects so `request_player_roll` PWs match CP Phantom (§9 test 3). Reaction = Drive+Reflexes per the code, not the book.
> - **On-demand NPC generation** (`src/lib/rules/generate.ts`) — ported `computeAttributes` / `distribute` / `ARCHETYPES` / `TIER_SCALE` + `processGeneratedEnemy` math. `generate_npc` tool (concept-only input, catalog text lifted verbatim from CP Phantom's `GEN_SYSTEM`); the model must generate an NPC's block before its first roll and reuse the cached numbers after.
> - Campaign State zod schema + structured-delta applier; two-phase turn loop with four tools (`roll_dice` server-executed, `request_player_roll` suspends the turn across HTTP requests, `commit_turn` = narration+delta, `generate_npc`); fact-compression pass; Firebase layer hard-isolated to `soloCampaigns/` (own test file) + read-only CP Phantom import helper; localStorage fallback; dictation button; play UI with a PC-PW spot-check panel and a session-log roll-audit panel; `/api/turn` route holding the API key server-side.
>
> **Needs the user:** Vercel project + `ANTHROPIC_API_KEY` (in progress), Firebase config env + `soloCampaigns/` rules (done), then the live end-to-end playtest — the gate before Phase 2.
> **Deferred to Phase 2+:** life-path creator, campaign bible, GM push-back pipeline, live Firebase import, conditional-talent toggle UI, NPC weapon-tech/attachment rolling, full visual pass (Phase 3).
> Narrator model defaults to `claude-sonnet-5` (env `SOLO_MODEL`) for per-turn cost.
>
> A shared `rules-core.js` that both tools load (instead of the solo tool re-porting) was proposed — real anti-drift fix, but a refactor of CP Phantom's live single-file app; not started, awaiting the user's call.
