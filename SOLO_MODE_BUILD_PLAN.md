...
> **Phase 1 build status (2026-08-31):** scaffolded in repo `JuliusBiTS/CP-Phantom-Game`. Done (41 passing tests, `tsc`/`build`/`lint` clean): real dice engine (`src/lib/dice/rollPW.ts`) incl. the v12 §2.1 capped-remainder example + a CSPRNG distribution check; v12 §7.7 PW-halving-stack + range bands (`src/lib/rules/pw.ts`); ported `getWoundState` house rule; **ported live PC-PW math** (`src/lib/rules/live.ts` — `getEffectiveStats`/`computeLiveWeaponStats`/`computeLiveReactionStats` + the weapon/cyberware/armor/Smart-tier catalogs), assembled into a `pcPwReference` block the turn prompt injects so `request_player_roll` PWs match CP Phantom (§9 test 3); Campaign State zod schema + structured-delta applier; the two-phase turn loop with the three tools (`roll_dice` server-executed, `request_player_roll` suspends the turn, `commit_turn` carries narration+delta); fact-compression pass; Firebase layer hard-isolated to `soloCampaigns/` (guard has its own test) with a read-only CP Phantom character-import helper; localStorage fallback store; dictation button; minimal play UI incl. a spot-check PW panel; `/api/turn` route holding the API key server-side. **Not yet done / needs the user:** Vercel project + `ANTHROPIC_API_KEY` env var (done by the user), Firebase config env vars + `soloCampaigns/` rules (done by the user), a real end-to-end play test against the live API. **Still deferred to Phase 2+:** on-demand NPC stat-block generation (port `processGeneratedEnemy`), conditional-talent toggle UI, life-path creator, campaign bible, GM push-back pipeline. Narrator model defaults to `claude-sonnet-5` (env `SOLO_MODEL`) to keep per-turn cost down.
